...
def main() -> None:
    ...
    rows = []
    preds = {}
    first_seed = cfg["split"]["seeds"][0]
    for name in M.CLASSICAL:
        for seed in cfg["split"]["seeds"]:
            # Keep the first seed's predictions so the write-up can show
            # predicted-vs-actual and a residual distribution, not only summary
            # statistics. They come back from the same fit - refitting just to
            # obtain them would double the cost of this step.
            keep = seed == first_seed
            out = M.fit_evaluate(name, Xd, yd, Xh, yh, seed=seed, fold=-1,
                                 arm="final_holdout", keep_predictions=keep)
            res, pred = out if keep else (out, None)
            if keep:
                preds[name] = pred
