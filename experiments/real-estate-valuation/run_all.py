...
Prerequisites, both manual because the sources are credential-gated:
  ...
  datasets/ch/central_bank_hpi/raw/*.xlsx                 BCCh IPV cuadros
  experiments/real-estate-valuation/.env                   TABPFN_TOKEN=<key>
...
STEPS = [
    ...
    ("Build BCCh IPV table", REPO_ROOT / "datasets/ch/central_bank_hpi/load.py", []),
    ("Verify leakage checks fire", HERE / "test_leakage.py", []),
    ("Feature diagnostics (correlation, collinearity)", HERE / "diagnostics.py", []),
...
def main() -> None:
    ...
    ap.add_argument("--skip-data", action="store_true",
                    help="reuse the processed dataset tables (raw files unchanged)")
    args = ap.parse_args()

    t_start = time.perf_counter()
    for i, (label, script, extra) in enumerate(STEPS, 1):
        if args.skip_data and script.parent.name in ("sii_cadastre", "central_bank_hpi"):
            print(f"\n[{i}/{len(STEPS)}] SKIPPED (--skip-data): {label}")
            continue
